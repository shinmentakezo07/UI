...
export function ModelCard({ model, index, onClick, featured = false }: ModelCardProps) {
                ...
                <div className="relative h-full bg-[#0A0A0A] rounded-[20px] p-6 flex flex-col border border-white/5 z-10">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div className="relative w-12 h-12 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center group-hover:scale-110 transition-transform duration-500">
                                {model.logo ? (
                                    ...
                                    <IconComponent className={`w-6 h-6 ${model.color || "text-gray-400"}`} />
                                )}
                            </div>

                            <div>
                                <h3 className={`font-bold tracking-tight text-white group-hover:text-blue-400 transition-colors line-clamp-1 ${featured ? "text-xl md:text-2xl" : "text-base"}`}>
                                    {model.name}
                                </h3>
                                <p className="text-xs text-gray-500 font-mono mt-0.5">{model.provider}</p>
                            </div>
                        </div>
                    ...
                    <div className="space-y-2.5 pt-4 border-t border-white/5 mb-5">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <span className="text-violet-400 font-mono font-bold text-sm">{model.outputPrice}/1M</span>
                        </div>
                    </div>
